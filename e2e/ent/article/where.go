...
func Published(v bool) predicate.Article {
	return predicate.Article(sql.FieldEQ(FieldPublished, v))
}
...
func PublishedNEQ(v bool) predicate.Article {
	return predicate.Article(sql.FieldNEQ(FieldPublished, v))
}
...
// HasArticleTag applies the HasEdge predicate on the "article_tag" edge.
func HasArticleTag() predicate.Article {
	return predicate.Article(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasArticleTagWith applies the HasEdge predicate on the "article_tag" edge with a given conditions (other predicates).
func HasArticleTagWith(preds ...predicate.ArticleTag) predicate.Article {
	return predicate.Article(func(s *sql.Selector) {
		step := newArticleTagStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
