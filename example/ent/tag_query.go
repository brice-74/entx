...
type TagQuery struct {
	...
	withArticleTag *ArticleTagQuery
...
// QueryArticleTag chains the current query on the "article_tag" edge.
func (tq *TagQuery) QueryArticleTag() *ArticleTagQuery {
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withArticleTag: tq.withArticleTag.Clone(),
...
// WithArticleTag tells the query-builder to eager-load the nodes that are connected to
// the "article_tag" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TagQuery) WithArticleTag(opts ...func(*ArticleTagQuery)) *TagQuery {
	query := (&ArticleTagClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withArticleTag = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withArticleTag; query != nil {
		if err := tq.loadArticleTag(ctx, query, nodes,
			func(n *Tag) { n.Edges.ArticleTag = []*ArticleTag{} },
			func(n *Tag, e *ArticleTag) { n.Edges.ArticleTag = append(n.Edges.ArticleTag, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadArticles(ctx context.Context, query *ArticleQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Article)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(article.FieldID), joinT.C(tag.ArticlesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.ArticlesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ArticlesPrimaryKey[0]))
...
func (tq *TagQuery) loadArticleTag(ctx context.Context, query *ArticleTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *ArticleTag)) error {
	...
	query.Where(predicate.ArticleTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.ArticleTagColumn), fks...))
	}))
...
func (q *TagQuery) AppendInterceptors(inters ...Interceptor) *TagQuery {
